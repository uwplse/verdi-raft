...
module VarDArrangement (P : VarDParams) = struct
  type name = VarDRaftDiskless.name
  type state = raft_data0
  type input = VarDRaftDiskless.raft_input
  type output = VarDRaftDiskless.raft_output
  type msg = VarDRaftDiskless.msg
  type res = (VarDRaftDiskless.raft_output list * raft_data0) * ((VarDRaftDiskless.name * VarDRaftDiskless.msg) list)
  type client_id = int
  ...
  let deserialize_msg = VarDDisklessSerialization.deserializeMsg
  let serialize_msg = VarDDisklessSerialization.serializeMsg
  let deserialize_input = VarDDisklessSerialization.deserializeInput
  let serialize_output = VarDDisklessSerialization.serializeOutput
  let debug = P.debug
